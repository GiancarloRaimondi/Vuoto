...
st.caption("Compatibile con report 'Categoria prodotto'. Collega automaticamente i tuoi file universali Fideuram.")
    ...
    if "ISIN" not in cols:
        for c in u.columns:
            if u[c].astype(str).str.match(r"^[A-Z]{2}[A-Z0-9]{10}$", na=False).any():
                ...
                break
    keep = []
    for k in ["ISIN","Categoria","Valuta","Area","Settore","Rating","Quartile","Costo_annuo_%","Margine_bps"]:
        if k in cols:
            keep.append(k)
    out = pd.DataFrame()
    for k in keep:
...
def export_pdf(metrics: dict) -> bytes:
    ...
    y -= 0.6*cm
    c.drawString(2*cm, y, f"Totale controvalore: € {metrics['totale']:,.2f}")
    y -= 0.6*cm
    c.drawString(2*cm, y, f"Margine annuo stimato: € {metrics['margine_eur']:,.2f}")
    y -= 0.6*cm
    c.drawString(2*cm, y, f"Costi annui stimati: € {metrics['costi_eur']:,.2f}")
    y -= 1.0*cm
    c.setFont("Helvetica-Bold", 12)
    c.drawString(2*cm, y, "Bandierine di revisione")
    y -= 0.6*cm
    c.setFont("Helvetica", 10)
    for line in metrics["flags_text"]:
        c.drawString(2.2*cm, y, f"- {line}")
        y -= 0.5*cm
        if y < 2*cm:
...
with st.sidebar:
    ...
    if src == "Carica XLSX":
        ...
        st.info("Carica un sample in sample_data/Portafoglio.xlsx se vuoi.")
...
if st.button("Esporta Excel"):
    ...
    st.download_button("Download Analisi.xlsx", data=data, file_name="Analisi_Preset.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
...
def export_pdf(metrics: dict) -> bytes:
    ...
    y -= 0.6*cm
    c.drawString(2*cm, y, f"Margine annuo stimato: € {margine_eur:,.2f}")
    y -= 0.6*cm
    c.drawString(2*cm, y, f"Costi annui stimati: € {costi_eur:,.2f}")
    y -= 1.0*cm
    c.setFont("Helvetica-Bold", 12)
    c.drawString(2*cm, y, "Bandierine di revisione")
    y -= 0.6*cm
    c.setFont("Helvetica", 10)
    for line in [f"{k}: {len(v)} strumenti" for k,v in flags.items()]:
        c.drawString(2.2*cm, y, f"- {line}")
        y -= 0.5*cm
        if y < 2*cm:
            c.showPage(); y = h - 2*cm; c.setFont("Helvetica", 10)
...
if st.button("Esporta PDF"):
    ...
    st.download_button("Download Report.pdf", data=pdf, file_name="Report_Preset.pdf", mime="application/pdf")

st.caption("Inserisci in `universali/` i tuoi Excel reali: Fondi/Foglio/Provvigioni. L’app li aggancia per ISIN e colonna alias.")
